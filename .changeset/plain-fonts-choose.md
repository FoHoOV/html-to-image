...
breaking: replace `skipFonts` and `fontEmbedCSS` with a single discriminated `fonts` option: `{ strategy: 'discover' }` (the default), `{ strategy: 'provided', css }`, or `{ strategy: 'none' }`. `skipFonts: true` becomes `fonts: { strategy: 'none' }`; `fontEmbedCSS: css` becomes `fonts: { strategy: 'provided', css }`.
